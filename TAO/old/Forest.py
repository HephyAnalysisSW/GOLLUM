...
class Forest:
    ...
    def predict(self, X, summed=True):
        ...
        """
        if summed:
            return np.sum(np.array([tree.predict(self.standardize_input(X)) for tree in self.trees]), axis=0)
        else:
            return np.array([tree.predict(self.standardize_input(X)) for tree in self.trees])
    ...
    def train_step(self, X, y, w, train_config):
        ...
        for i_tree, tree in enumerate(self.trees):
            ...
            #print("Subtracting: ", i_tree, tree.predict(std_inputs))
            #print("Forest predictions:", i_tree, predictions)
            tree.train_step( X=self.standardize_input(X), y=y-predictions, w=w, train_config=train_config)
            #print("Adding: ", i_tree, tree.predict(std_inputs))
            predictions += tree.predict(std_inputs)
            #print("Now: ", i_tree, predictions)
