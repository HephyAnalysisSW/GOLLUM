import sys
import os
sys.path.insert(0, '..')
sys.path.insert(0, '../..')

import ROOT
import numpy as np
import common.syncer  
import common.helpers

dir_path = os.path.dirname(os.path.realpath(__file__))
ROOT.gROOT.LoadMacro(os.path.join(dir_path, "../../common/scripts/tdrstyle.C"))
ROOT.setTDRStyle()

def create_calibration_graph(values, labels, weights,
                             class_of_interest=0, nbins=10):
    """
    Create a TGraph for a weighted "calibration-like" plot of a single class among multiple classes.
    
    In each bin, we compute:
      - x = ( sum_{i in bin}[w_i * values_i] ) / ( sum_{i in bin}[w_i] )
            (the weighted mean of the predicted dcr in that bin)
      - y = ( sum_{i in bin}[w_i] for events with label == class_of_interest ) / ( sum_{i in bin}[w_i] )
            (the weighted true dcr in that bin)
    
    If a bin is empty, we place x at the bin center and set y = 0.
    
    Parameters
    ----------
    values : np.ndarray of shape (N,)
        The predicted dcr values for the x-axis (not necessarily in [0,1]).
    labels : np.ndarray of shape (N,)
        Integer class labels for each event.
    weights : np.ndarray of shape (N,)
        Event cross-section weights (nonnegative; not normalized when dcr is True).
    class_of_interest : int
        Which class you want to compute the true dcr for.
    nbins : int
        Number of bins in [min(values), max(values)].
    
    Returns
    -------
    graph : ROOT.TGraph
        A TGraph with nbins points: (weighted mean of predicted dcr, weighted true dcr).
    """
    # Ensure arrays are NumPy arrays
    values = np.asarray(values, dtype=float).flatten()
    labels = np.asarray(labels, dtype=int).flatten()
    weights = np.asarray(weights, dtype=float).flatten()
    assert len(values) == len(labels) == len(weights), \
        "values, labels, and weights must all have the same length."

    if len(values) == 0:
        return ROOT.TGraph(0)

    v_min, v_max = values.min(), values.max()
    if v_min == v_max:
        true_dcr = float(np.sum(weights[labels == class_of_interest])) / np.sum(weights) if np.sum(weights) > 0.0 else 0.0
        g = ROOT.TGraph(1)
        g.SetPoint(0, v_min, true_dcr)
        return g

    # Compute binned sums
    bin_sum_w, bin_edges = np.histogram(
        values,
        bins=nbins,
        range=(v_min, v_max),
        weights=weights
    )
    mask_class = (labels == class_of_interest)
    bin_sum_wclass, _ = np.histogram(
        values[mask_class],
        bins=nbins,
        range=(v_min, v_max),
        weights=weights[mask_class]
    )
    bin_sum_wvalue, _ = np.histogram(
        values,
        bins=nbins,
        range=(v_min, v_max),
        weights=weights * values
    )

    graph = ROOT.TGraph(nbins)
    for i in range(nbins):
        bin_left = bin_edges[i]
        bin_right = bin_edges[i+1]
        bin_center = 0.5*(bin_left + bin_right)

        w_tot = bin_sum_w[i]
        if w_tot > 0.0:
            x_mean = bin_sum_wvalue[i] / w_tot
            true_dcr = bin_sum_wclass[i] / w_tot
        else:
            x_mean = bin_center
            true_dcr = 0.0

        graph.SetPoint(i, x_mean, true_dcr)

    return graph

import common.user as user
import common.data_structure as data_structure
import common.selections as selections
import common.datasets_hephy as datasets_hephy

small = True
# Calibrate DCR (differential cross section ratio) or Prob?
dcr = True      # <<< Set dcr to True
nbins = 50

soft_colors = [
    ROOT.TColor.GetColor("#779ECB"),
    ROOT.TColor.GetColor("#03C03C"),
    ROOT.TColor.GetColor("#B39EB5"),
    ROOT.TColor.GetColor("#FFB347"),
    ROOT.TColor.GetColor("#FFD1DC"),
    ROOT.TColor.GetColor("#AEC6CF"),
    ROOT.TColor.GetColor("#CFCFC4"),
    ROOT.TColor.GetColor("#77DD77")
]

if small:
    n_split = 100
else:
    n_split = 1

from ML.TFMC.TFMC import TFMC
tfmc = TFMC.load("/groups/hephy/cms/robert.schoefbeck/Challenge/models/TFMC/lowMT_VBFJet/tfmc_2_reg/v6")

# Note: In dcr mode, we do not normalize the weights.
# The following normalization block is skipped when dcr is True.
if not dcr:
    weights[labels == 0] /= np.sum(weights[labels == 0])
    weights[labels == 1] /= np.sum(weights[labels == 1])
    weights[labels == 2] /= np.sum(weights[labels == 2])
    weights[labels == 3] /= np.sum(weights[labels == 3])

# Load one batch from the dataset for demonstration
loader = datasets_hephy.get_data_loader(selection="lowMT_VBFJet", n_split=n_split)
for batch in loader:
    data, weights, labels = loader.split(batch)
    print(data.shape, weights.shape, labels.shape, np.unique(labels, return_counts=True))
    prob_tf = tfmc.predict(data, ic_scaling=dcr)  # returns predicted dcr when dcr=True
    break

region = "lowMT_VBFJet"
from ML.Calibration.MulticlassCalibration import MultiClassCalibration
mc_calib = MultiClassCalibration.load(f"/groups/hephy/cms/robert.schoefbeck/Challenge/models/Calibration/{region}/config_reference_v3/{region}/calibrator_multi.pkl")
from ML.Calibration.Calibration import Calibration
calib = Calibration.load(f"/groups/hephy/cms/robert.schoefbeck/Challenge/models/Calibration/{region}/config_reference_v2_calib/{region}/calibrator.pkl")

# Create a canvas with 2x2 pads
c = ROOT.TCanvas("c", "Calibration Plots", 1200, 900)
c.Divide(2, 2)

graphs_tf = []
graphs_xgb = []

stuff = []
for j in range(4):
    c.cd(j+1)
    # Create the calibration graph for class j.
    g_tf = create_calibration_graph(prob_tf[:, j], labels, weights, class_of_interest=j, nbins=nbins)
    color = soft_colors[0]

    g_tf.SetLineColor(color)
    g_tf.SetMarkerColor(color)
    g_tf.SetMarkerStyle(20)
    g_tf.SetLineWidth(2)

    # Use a title appropriate for dcr calibration.
    if dcr:
        g_tf.SetTitle(";Mean Predicted DCR;True DCR")
    else:
        g_tf.SetTitle(";Mean Predicted Probability;Weighted True Probability")

    g_tf.Draw("ALP")
    # If dcr values are fractions, you might still set [0,1]. Otherwise adjust the range as needed.
    g_tf.GetXaxis().SetRangeUser(0.0, 1.0)
    g_tf.GetYaxis().SetRangeUser(0.0, 1.0)

    line = ROOT.TLine(0, 0, 1, 1)
    line.SetLineColor(ROOT.kGray+2)
    line.SetLineStyle(9)
    line.Draw("SAME")
    stuff.append(line)

    legend = ROOT.TLegend(0.2, 0.75, 0.55, 0.85)
    legend.SetBorderSize(0)
    legend.SetShadowColor(0)
    legend.AddEntry(g_tf, "Model TF", "lp")
    legend.Draw("SAME")
    stuff.append(legend)

    graphs_tf.append(g_tf)

c.RedrawAxis()
c.Update()

# Save the canvas; filenames now depend on whether we are in dcr mode.
if dcr:
    c.Print(os.path.join(user.plot_directory, "calib", ("small_" if small else "") + "calib_dcr.pdf"))
    c.Print(os.path.join(user.plot_directory, "calib", ("small_" if small else "") + "calib_dcr.png"))
else:
    c.Print(os.path.join(user.plot_directory, "calib", ("small_" if small else "") + "calib_prob.pdf"))
    c.Print(os.path.join(user.plot_directory, "calib", ("small_" if small else "") + "calib_prob.png"))

common.helpers.copyIndexPHP(os.path.join(user.plot_directory, "calib"))
common.syncer.sync()

