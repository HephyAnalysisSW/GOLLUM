#!/usr/bin/env python3
"""
Plot PDF curves from toy best fits for the direct-reweighting toy workflow.

The plot contains:
- the reference PDF from the config
- the target PDF used in generate_toys_v3.py
- the mean toy-fit curve
- the 68% toy band

The ratio panel uses f / f_ref.
"""
...
def load_toy_metadata(toys_npz_path):
...
def evaluate_target_pdf(target_set, target_member, x_vals, pid, Q):
...
def build_pdf_band(config, fit_dir, rotate_json=None, Q=70.0, pid=21, x_min=0.003, x_max=0.6, n_x=200):
    fit_poi_names, toy_ids, chat, n2ll_min, fit_meta = load_fit_results_dir(fit_dir)
    toy_meta = load_toy_metadata(fit_meta["toys_npz"])
    ...
    # Reference curve from the config PDF corresponds to coeffs = 0.
    ref_coeffs = np.zeros(len(base_poi_names), dtype=float)
    ref_curve = pdf.evaluate(x=x_vals, id=id_arr, Q=Q_arr, coeffs=ref_coeffs)

    target_set = toy_meta.get("target_pdf_set")
    target_member = toy_meta.get("target_member")
    ...
    target_curve = evaluate_target_pdf(target_set, target_member, x_vals, pid, Q)
    ...
    mask = target_curve != 0.0
    r_target = np.ones_like(target_curve)
    r_mean = np.ones_like(target_curve)
    r16 = np.ones_like(target_curve)
    r84 = np.ones_like(target_curve)
    r_target[mask] = target_curve[mask] / target_curve[mask]
    r_mean[mask] = toy_mean[mask] / target_curve[mask]
    r16[mask] = p16[mask] / target_curve[mask]
    r84[mask] = p84[mask] / target_curve[mask]

    target_label = f"{target_set} member {int(target_member)}"
    ref_label = pdf_cfg.get("pdf_basis")
    return x_vals, ref_curve, target_curve, toy_mean, p16, p84, r_target, r_mean, r16, r84, target_label, ref_label
...
def plot_pdf_band(x, ref_curve, target_curve, toy_mean, p16, p84, r_target, r_mean, r16, r84,
                  Q, outname, target_label, ref_label):
    ...
    ax.plot(x, ref_curve, color="#d62728", lw=1.8, ls="--", label=ref_label)
    ax.plot(x, target_curve, color="#1f77b4", lw=1.8, label=target_label)

    axr.fill_between(x, r16, r84, alpha=0.25)
    axr.axhline(1.0, color="black", lw=1.2)
    axr.plot(x, r_mean, color="black", lw=1.6)
    axr.plot(x, r_target, color="#1f77b4", lw=1.4)

    ax.set_ylabel(rf"$f(x,Q = {float(Q)})$")
    axr.set_xlabel(r"$x$")
    axr.set_ylabel(r"$f/f_{\mathrm{target}}$", fontsize=12)
...
def main():
    ...
    parser.add_argument("--x-min", type=float, default=0.003, help="Minimum x")
    parser.add_argument("--x-max", type=float, default=0.6, help="Maximum x")
    parser.add_argument("--n-x", type=int, default=200, help="Number of x points")
    parser.add_argument("--outname", default=None, help="Output figure stem")
    args = parser.parse_args()

    x_vals, ref_curve, target_curve, toy_mean, p16, p84, r_target, r_mean, r16, r84, target_label, ref_label = build_pdf_band(
        args.config,
        args.fit_dir,
