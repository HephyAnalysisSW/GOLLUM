...
if __name__ == '__main__':
    ...
    parser.add_argument("--asimov_nu_tt", type=float, default=None, help="Fix asimov nu_ttbar.")
    ...
    plot_directory   = os.path.join(user.plot_directory,   "CSIplots", config_name)
    ...
    ll_CSI = lambda mu, nu_bkg, nu_tt, nu_diboson, nu_tes, nu_jes, nu_met: \
        infer_CSI.incS_diff_from_csis( args.selection, mu=mu, nu_bkg=nu_bkg, nu_tt=nu_tt, nu_diboson=nu_diboson, nu_tes=nu_tes, nu_jes=nu_jes, nu_met=nu_met)
    ...
    #mg.Add(gr_noCSI, "LP")
    mg.Add(gr_CSI,   "LP")
    mg.SetTitle(f"xsec term vs {var_to_scan};{var_to_scan};Log-Likelihood")
    ...
    y_all = np.concatenate([y_vals_CSI_arr])
    ...
    png_name = os.path.join(plot_directory,
                f"likelihoodScan_{args.selection}_{var_to_scan}_{postfix}.png" if postfix else
                f"likelihoodScan_{args.selection}_{var_to_scan}.png")
    c.SaveAs(png_name)
