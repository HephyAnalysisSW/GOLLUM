...
import common.derivative_providers as derivative_providers
...
if ich is None or args.overwrite:
    ...
    ich = InclusiveCrosssectionHistogram(
        variables=derivative_provider.parameters,
        combinations=derivative_provider.combinations,
    ...
    if selection_name is not None:
        # not sure why this line was here, likely some things that were not pushed
        # bypassing the error for those of us who don't have that module
        try:
            sel_mod = importlib.import_module("common.selections")
            sel_fn = sel_mod.selections.get(selection_name, None)
            print("Using common.selections for named selections.")
            if sel_fn is None:
    ...
    for shard in tqdm(range(n_shards), desc="Processing shards"):
        ...
        if derivative_provider:
            derivs = derivative_provider.truth_weight_matrix(G, w, obs_names)
            ich.accumulate(axis1_vals, derivs, axis2_vals)

            ich.expansion_point = derivative_provider.expansion_point
            print(f"{ich.expansion_point=}")
