...
# ---------------- UID splitting (YAML-driven, implemented in samples_RunII.py) ----------------
UID_CFG = (CFG.get("uid_splitting") or {})
...
uid_assign   = (UID_CFG.get("assignment") or {})

def _build_bucket_intervals(scheme: dict, n_buckets: int) -> dict:
    """
    Convert scheme fractions to deterministic bucket intervals [lo, hi).
    Uses YAML insertion order. Floors each part and assigns remainder to the last.
    """
    if n_buckets <= 0:
        raise ValueError(f"n_buckets must be positive, got {n_buckets}")
    if not scheme:
        raise ValueError("uid_splitting.scheme is empty")

    keys = list(scheme.keys())
    fracs = [float((scheme[k] or {}).get("fraction", 0.0)) for k in keys]
    s = sum(fracs)
    if not (0.999 <= s <= 1.001):
        raise ValueError(f"uid_splitting.scheme fractions must sum to 1, got {s}")

    sizes = [int(math.floor(f * n_buckets)) for f in fracs]
    rem = n_buckets - sum(sizes)
    sizes[-1] += rem
    ...
    # sanity
    if lo != n_buckets:
        raise RuntimeError(f"bucket interval packing failed: ended at {lo}, expected {n_buckets}")
    return out

uid_intervals = None
uid_splitter = None
if uid_enabled:
    if not hasattr(samples_mod, "UIDSplitter"):
        raise RuntimeError(
            f"uid_splitting.enabled=True but module {module_samples} has no UIDSplitter. "
        )
    uid_splitter = samples_mod.UIDSplitter(
        uid_fields=tuple(uid_fields),
        seed=uid_seed,
        n_buckets=uid_n_buckets,
    )
    uid_intervals = _build_bucket_intervals(uid_scheme, uid_n_buckets)

    # resolve PNN train/val split names from assignment
    try:
        pnn_train_key = uid_assign["pnn"]["train"]
        pnn_val_key   = uid_assign["pnn"]["validation"]
    except Exception:
        raise ValueError("uid_splitting.assignment.pnn.train/validation missing in YAML")

    if pnn_train_key not in uid_intervals:
        raise ValueError(f"assignment.pnn.train='{pnn_train_key}' not in uid_splitting.scheme keys")
    if pnn_val_key not in uid_intervals:
        raise ValueError(f"assignment.pnn.validation='{pnn_val_key}' not in uid_splitting.scheme keys")
...
def _wrap_with_uid_mask(obj, interval, split_key):
    lo, hi = interval

    def _sel(G_sel, names):
        return uid_splitter.mask_from_np(G_sel, list(names), lo, hi)

    if isinstance(obj, RDataLoader):
        base_name = getattr(obj, "name", None) or obj.__class__.__name__
        return SelectionView(
            base=obj,
            name=f"{base_name}__{split_key}",
            selection_fn=_sel,
            feature_names=obj.feature_names,
            observer_names=obj.observer_names,
            selection_feature_names=obj.observer_names,  # VERY IMPORTANT
            weight=None
        )

    if isinstance(obj, SelectionView):
        prev = obj._selection_fns
        if prev is None:
            combined = _sel
...
if uid_enabled:
    train_loaders = [_wrap_with_uid_mask(L, train_interval, pnn_train_key) for L in loaders]
    val_loaders   = [_wrap_with_uid_mask(L, val_interval, pnn_val_key)   for L in loaders]
else:
    train_loaders = list(loaders)
    val_loaders   = list(loaders)
...
print(f"[SHARDS] train: {n_shards_train} shards -> 0..{n_shards_train-1 if n_shards_train else 'N/A'}")
print(f"[SHARDS] val  : {n_shards_val} shards -> 0..{n_shards_val-1 if n_shards_val else 'N/A'}")


# ---------------- quick check: event counts (nominal bp) ----------------
n_train_events = 0
for Xs, _Ws in iterate_epoch(train_loaders, shard_limit=shard_limit):
    n_train_events += int(len(Xs[nom_idx]))

n_val_events = 0
for Xs, _Ws in iterate_epoch(val_loaders, shard_limit=shard_limit):
...
ES_CFG = ((CFG.get("pnn_training") or {}).get("early_stopping") or {})
...
for epoch in trange(start_epoch, epochs, desc="Epoch"):
    ...
    for Xs, Ws in tqdm(iterate_epoch(train_loaders, shard_limit=shard_limit), desc="Train", unit="shard"):
        n_train_shards += 1
    ...
    for Xs, Ws in tqdm(iterate_epoch(val_loaders, shard_limit=shard_limit), unit="shard"):
        n_val_shards += 1
