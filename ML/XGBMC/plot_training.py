...
argParser.add_argument("--modelDir", action="store", required=True, help="Directory containing the trained XGBMC model.")
...
# Load the trained XGBMC model
from XGBMC import XGBMC
xgbmc = XGBMC.load(args.modelDir)
...
plot_directory = os.path.join(user.plot_directory, "XGBMC", args.selection, args.config, "predicted_probabilities")
...
histograms = {c: {label: np.zeros(n_bins) for label in data_structure.labels} for c in range(xgbmc.num_classes)}
...
for i_batch, batch in enumerate(tqdm(data_loader, total=total_batches, desc="Batches")):
    ...
    predictions = xgbmc.predict(features, ic_scaling=False)

    # Accumulate predicted probabilities for each class and label
    for c in range(xgbmc.num_classes):
...
for c, predicted_class in enumerate(xgbmc.classes):
