...
def process_final_data(final_data):
    ...
    column_order = [
        ...
        'DER_pt_tot', 'DER_sum_pt', 'DER_pt_ratio_lep_had', 'DER_met_phi_centrality', 
        'DER_lep_eta_centrality'
    ]
    
    # 检查final_data是否包含所需列
    if not all(col in final_data.columns for col in column_order + ['detailed_labels']):
        missing_cols = [col for col in column_order + ['detailed_labels'] if col not in final_data.columns]
        raise ValueError(f"Missing columns in final_data: {missing_cols}")

    final_data['Constant_1'] = 1

    # 按顺序重新排列列
    final_column_order = column_order + ['Constant_1', 'detailed_labels']
    ordered_data = final_data[final_column_order].copy()
