...
class AnalyticPDFParametrization:
    """
    ...
    def __init__(self, n: int = 5, typ: str="Chebyshev"):
        ...
        if typ == "Bernstein":
            self.bernstein_basis = BernsteinBasis(self.n)
    ...
    def evaluate(self, x: ArrayLike, id: ArrayLike, coeffs: Sequence[float]) -> ArrayLike:
        ...
        if self.typ == "Chebyshev":
            ...
            V = self.bernstein_basis.values(y_vals)
        else:
            raise ValueError("Unknown PDF type {self.typ}")
    ...
    def product_parametrizations(self, x1, x2, id1, id2, coeffs, Q=None):
    ...
    def derivatives(self, x1, x2, id1, id2, Q=None):
        """
        ...
        if self.typ == "Chebyshev":
            ...
            V1 = self.bernstein_basis.values(y1) 
            V2 = self.bernstein_basis.values(y2) 
        else:
            raise ValueError("Unknown PDF type {self.typ}") 
...
if __name__ == "__main__":
    ...
    pdf0 = AnalyticPDFParametrization(n=0, typ=use_basis)
    ...
    pdf = AnalyticPDFParametrization(n=3, typ=use_basis)
