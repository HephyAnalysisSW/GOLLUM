...
model_directory = os.path.join(user.model_directory, version)
os.makedirs(model_directory, exist_ok=True)
print(f"[info] Model directory: {model_directory}")

# Write patched YAML into model_directory
yaml_basename   = os.path.basename(args.config)
patched_yaml_path = os.path.join(model_directory, yaml_basename)
...
###############
#LB: fit results are stored as json files in output directory
#LB: here we open the json file according to the 
...
if args.rotate:
    syst_suffix  += "_rotate"
print(user.output_directory,f"{base}_{base_version}{syst_suffix}_fit.json")
#the name is the cfg name and version and json
...
if os.path.isfile(output_json):
    ...
    # output directory for postfit plots
    postfit_plot_directory = os.path.join(user.plot_directory, 'postfit_stacks')
    os.makedirs(postfit_plot_directory, exist_ok=True)
    print(f"[info] Postfit plots will be written under: {postfit_plot_directory}")
...
toy_mean = np.mean(toys, axis=0)
for name, mval, tval in zip(param_order, params_best, toy_mean):
    print(f"{name:25s} best-fit={mval: .8f}   toy-mean={tval: .8f}")
toy_cov = np.cov(toys, rowvar=False)
...
# ---------------- prefit stack plots ----------------
# knobs
n_toys_prefit          = 10000  # number of random nuisance samples
...
print(theta_samples[:2])
print(toys[:2])
print("Put the postfit toys here?")

pre_toy_mean = np.mean(theta_samples, axis=0)
pre_toy_cov = np.cov(theta_samples, rowvar=False)
...
print("cov post toys:")
print(toy_cov[0])
print("Original minuit covariance:")
print(cov[0])
