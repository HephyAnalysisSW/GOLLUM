...
# ----------------------------------------------------------------------
# UID fields: always load these as observers so training scripts can apply
# deterministic UID-based masks without touching RDataLoader.py.
# NOTE: samples_RunII.py stays YAML-agnostic; no fractions/scheme here.
# ----------------------------------------------------------------------
UID_FIELDS_DEFAULT = ("run", "luminosityBlock", "event")

# Merge UID fields into OBSERVERS (order-preserving de-dup)
OBSERVERS_WITH_UID = list(dict.fromkeys(list(observables.OBSERVERS) + list(UID_FIELDS_DEFAULT)))
...
_base = RDataLoader(
    ...
    observer_names=observables.OBSERVERS_WITH_UID,
)
...
class UIDSplitter:
    ...
    def __init__(
        self,
        uid_fields: tuple[str, str, str] = UID_FIELDS_DEFAULT,
        seed: int = 0,
        n_buckets: int = 10000,
    ) -> None:
...
if __name__ == "__main__":
    ...
    assert "run" in _base.observer_names and "luminosityBlock" in _base.observer_names and "event" in _base.observer_names
    print("UID observers OK:", [k for k in UID_FIELDS_DEFAULT])